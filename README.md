...
- ### 코인 자동 선정
  ...
  - 현재는 과거 7일 간의 데이터를 분석하여 최고의 수익률을 계산합니다. 만약 더 적거나 더 많은 일수의 데이터를 참조하고 싶으시면

    **calBestCoinAndK.py** 의 **get_best_ticker()** 함수 정의 부분에서 아래 코드 중 count 인자값을 변경해주면 됩니다.

    ```python
    df = pyupbit.get_ohlcv(t,count = 7) 
    ```

    현재는 count가 7이라 7일 데이터 기반으로 계산을 하고, count = 15로 설정하여 15일 데이터 기반으로 계산하게끔 만들어도 됩니다.
